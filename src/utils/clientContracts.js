...
export function createClientContract(raw = {}) {
  return {
    ...
    client_email: normalizeEmail(raw?.client_email || ''),
    ...
    issued_by_email: normalizeEmail(raw?.issued_by_email || ''),
    issued_by_name: String(raw?.issued_by_name || '').trim(),
    account_manager_name: String(raw?.account_manager_name || '').trim(),
    account_manager_email: normalizeEmail(raw?.account_manager_email || ''),
...
export function getClientContractStatusLabel(status = '') {
  ...
  if (safe === 'awaiting_client_signature') return ['Awaiting signature', 'pending']
  if (safe === 'completed') return ['Signed', 'complete']
  if (safe === 'voided') return ['Voided', 'cancelled']
